...
class SeekerApplicationListIntegrationTest {
    ...
    private final ConcurrentMap<UUID, JobApplication> applicationsBySeeker = new ConcurrentHashMap<>();
    ...
    void setUp() {
        ...
        when(userAccountRepository.findById(nonNullArgument())).thenAnswer(invocation -> {
            ...
            return seekerUser.getId().equals(userId)
                    ? java.util.Optional.of(seekerUser)
                    : java.util.Optional.empty();
        });

        when(jobApplicationRepository.findAllBySeekerIdOrderByCreatedAtDesc(nonNullArgument()))
                .thenAnswer(invocation -> {
                    UUID seekerId = invocation.getArgument(0);
                    return applicationsBySeeker.values().stream()
    ...
    @Test
    void seekerCanListOwnApplicationsWithJobDetails() throws Exception {
        Job job = buildJob(employerUser, "Backend Engineer", "Acme Corp");
        JobApplication application = buildApplication(job, seekerUser, ApplicationStatus.SUBMITTED,
                Instant.parse("2026-05-01T10:00:00Z"));
        applicationsBySeeker.put(application.getId(), application);
    ...
    @Test
    void seekerApplicationsIncludesDraftStatus() throws Exception {
        Job job = buildJob(employerUser, "Frontend Engineer", null);
        JobApplication draft = buildApplication(job, seekerUser, ApplicationStatus.DRAFT, null);
        applicationsBySeeker.put(draft.getId(), draft);
    ...
    @Test
    void listReturnsEmptyArrayWhenNoApplicationsExist() throws Exception {
        mockMvc.perform(get("/api/seeker/applications")
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + issueToken(seekerUser)))
                .andExpect(status().isOk())
    ...
    private Job buildJob(UserAccount employer, String title, String companyName) {
        Job job = new TestJob();
    ...
    private JobApplication buildApplication(Job job, UserAccount seeker, ApplicationStatus status, Instant submittedAt) {
